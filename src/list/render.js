...
const sortTasks = (tasksList) => {
  ...
  const tasksDone = sortTasksList.filter((task) => task.done === true);
  const tasksNotDone = sortTasksList.filter((task) => task.done === false);
  return [...tasksNotDone, ...tasksDone];
};

const createListElements = (tasksList) => tasksList.map(({ id, text, done }) => {
